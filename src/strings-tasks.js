...
function sumOfCodes(str) {
  if (typeof str === 'string') {
    const arr = [...str];
    const sum = arr.reduce((acc, current) => acc + current.charCodeAt(0), 0);
...
function isPalindrome(str) {
  if (str && str.length) {
    ...
    const revert = formattedStr.split('').reverse().join('');

    return formattedStr === revert;
  }
  return false;
}
...
function findLongestWord(sentence) {
  ...
  return sentenceArr.reduce((longWord, currentWord) => {
...
function reverseWords(str) {
  const arr = str.split(' ');
  const formattedArr = arr.map((item) => item.split('').reverse().join(''));
...
function encodeToRot13(str) {
  const rightAlphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const encodedAlphabet =
    'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const arr = str.split('');
  const newStr = arr.map((item) => {
    const findIndex = rightAlphabet.indexOf(item);
    if (findIndex === -1) {
      return item;
    }
    const encodedLetter = encodedAlphabet[findIndex];
...
function getCardId(value) {
  ...
  return cardsArr.findIndex((item) => item === value);
}
